...
//			//while(RESET == dma_flag_get(DMA0, DMA_CH6, DMA_INTF_FTFIF));	//等待通道7传输完成   
...
u8 GSM_Dect(void)
{
	...
	{
		case SIM_OK:
			Uart0Printf("GSM模块自检成功\r\n");
			break;
		case SIM_COMMUNTION_ERR:
			Uart0Printf("与GSM模块未通讯成功，请等待\r\n");
			break;
		case SIM_CPIN_ERR:
			Uart0Printf("没检测到SIM卡\r\n");
			break;
		case SIM_CREG_FAIL:
			Uart0Printf("注册网络中....\r\n\r\n");
			Uart0Printf("当前信号值：%s\r\n",SIM900_CSQ);
...
u8 SIM900A_CONNECT_SERVER_SEND_INFOR(u8 *IP_ADD,u8 *COM)
{
	...
	{
		case 1:
			Uart0Printf("SIM卡开启网络失败\r\n");
			break;
		case 2:
			Uart0Printf("透传模式设置失败\r\n");
			break;
		default:
			break;
	}
...
void sim7600ce_connect()
{
  ...
  //char TCP_Server_COM[] = "6668";			//TCP服务器端口
	  while(res)
	{
		...
		res = GSM_Dect();//GPRS模块自检
		nopdelay_ms(1500);
	}	
	
	SIM900A_CONNECT_SERVER_SEND_INFOR((u8*)TCP_Server_IP,(u8*)TCP_Server_COM);  //建立连接

	while(NET_flag != 1);	//等待网络链接上
	Uart0Printf("网络连接成功！\r\n");
	nopdelay_ms(1000);
}
...
void init_4g (void)
{
		nvic_priority_group_set(NVIC_PRIGROUP_PRE2_SUB2);//设置NVIC中断分组2:2位抢占优先级，2位响应优先级
	  USART2_Init(115200u);	
		sim7600ce_connect();//4G网络连接
