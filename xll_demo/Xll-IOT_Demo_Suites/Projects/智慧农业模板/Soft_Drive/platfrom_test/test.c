#include "gd32f4xx.h"
#include "gd32f4xx_XII-IOT.h"
#include "systick.h"
#include "test.h"
#include <stdio.h>
#include <stdlib.h>
#include "user.h"
#include "E53_IA1.h"
#include "I2C.h"
#include "oled.h"
#include "74HC595_LED.h"

E53_IA1_Data_TypeDef E53_IA1_Data;

#define all_led_on gd_eval_led_on(LED1);\
			gd_eval_led_on(LED2);\
			gd_eval_led_on(LED3);\
			gd_eval_led_on(LED4);\
			gd_eval_led_on(LED5);\
			gd_eval_led_on(LED6);\
			gd_eval_led_on(LED7);\
			gd_eval_led_on(LED8);\

#define all_led_off gd_eval_led_off(LED1);\
			gd_eval_led_off(LED2);\
			gd_eval_led_off(LED3);\
			gd_eval_led_off(LED4);\
			gd_eval_led_off(LED5);\
			gd_eval_led_off(LED6);\
			gd_eval_led_off(LED7);\
			gd_eval_led_off(LED8);\

float humd_ub = 80.0;
float humd_lb = 20.0;
float temp_ub = 30.0;
float temp_lb = 15.0;
float lux_ub = 300;
float lux_lb = 30;

uint8_t humd[16];
uint8_t temp[16];
uint8_t lux[16];
uint8_t blank[16] = "        ";

void BEEP(uint32_t t) {
	gpio_bit_set(GPIOD, GPIO_PIN_12);
	delay_1ms(t);
	gpio_bit_reset(GPIOD, GPIO_PIN_12);
	delay_1ms(t);
}

void BEEP_LONG_3() { // 3声长鸣
	uint8_t i;
	for(i = 0; i < 3; i++) BEEP(1000);
}


void BEEP_SHORT_3() { // 3声长鸣
	uint8_t i;
	for(i = 0; i < 3; i++) BEEP(100);
}

void PWM_ON() {
	uint8_t speed = 5;
	gpio_bit_reset(GPIOC, GPIO_PIN_6);
	gpio_bit_set(GPIOC, GPIO_PIN_7);
	gpio_bit_set(GPIOC, GPIO_PIN_8);
	gpio_bit_set(GPIOC, GPIO_PIN_9);
	delay_1ms(speed);
	gpio_bit_set(GPIOC, GPIO_PIN_6);
	gpio_bit_reset(GPIOC, GPIO_PIN_7);
	gpio_bit_set(GPIOC, GPIO_PIN_8);
	gpio_bit_set(GPIOC, GPIO_PIN_9);
	delay_1ms(speed);
	gpio_bit_set(GPIOC, GPIO_PIN_6);
	gpio_bit_set(GPIOC, GPIO_PIN_7);
	gpio_bit_reset(GPIOC, GPIO_PIN_8);
	gpio_bit_set(GPIOC, GPIO_PIN_9);
	delay_1ms(speed);
	gpio_bit_set(GPIOC, GPIO_PIN_6);
	gpio_bit_set(GPIOC, GPIO_PIN_7);
	gpio_bit_set(GPIOC, GPIO_PIN_8);
	gpio_bit_reset(GPIOC, GPIO_PIN_9);
	delay_1ms(speed);
}


//初始化 系统时钟、XLL\拨码开关、OLED、智慧农业模块等按需编写
void system_init(void) {
		systick_config(); // 初始化系统时钟
		gd_XII_systeminit(); // 初始化8个LED灯
	  dipinit(); // 初始化拨码开关
		OLED_Gpio_Init();
		OLED_Init();
		i2c_gpio_config();
		i2c_config();
		usart0Init(EVAL_COM0 ,115200U);
		Init_E53_IA1();
		HC595_GPIO_Config();
}

//程序编写
void test(void) {
	OLED_ShowString(0, 0, (uint8_t *)"Humd: ");
	OLED_ShowString(0, 2, (uint8_t *)"Temp: ");
	OLED_ShowString(0, 4, (uint8_t *)"Lux : ");
	while(1) {
		E53_IA1_Read_Data();
		
		Uart0Printf("Humidity:%3.lf, Temprature:%3.lf°C, Lux: %3.lf \r\n", 
		E53_IA1_Data.Humidity,
		E53_IA1_Data.Temperature,
		E53_IA1_Data.Lux);
		
		sprintf((char *)humd, "%3.lf%%", E53_IA1_Data.Humidity);
		sprintf((char *)temp, "%3.lf°C", E53_IA1_Data.Temperature);
		sprintf((char *)lux, "%3.lf", E53_IA1_Data.Lux);
		
		OLED_ShowString(42, 0, blank);
		OLED_ShowString(42, 2, blank);
		OLED_ShowString(42, 4, blank);
		OLED_ShowString(42, 0, temp);
		OLED_ShowString(42, 2, humd);
		OLED_ShowString(42, 4, lux);
		
		/*Humidity 操作*/
		if(E53_IA1_Data.Humidity > humd_ub) {
			E53_IA1_Motor_StatusSet(ON); // 湿度超过高阈值 打开电机
			OLED_ShowString(80, 0, (uint8_t*)"H");
		}
		else if(E53_IA1_Data.Lux > lux_ub) {
			E53_IA1_Motor_StatusSet(OFF); // 湿度低于低阈值 关闭电机
			OLED_ShowString(80, 0, (uint8_t*)"L");
		}
		else { // 处于两阈值之间
			E53_IA1_Motor_StatusSet(OFF);
			OLED_ShowString(80, 0, (uint8_t*)"Y"); // 表示正常
		}
		/*Humudity 操作结束*/
		
		/*Temprature 操作*/
		if(E53_IA1_Data.Temperature > temp_lb) {
			OLED_ShowString(80, 2, (uint8_t*)"H");
		}
		else if(E53_IA1_Data.Lux > lux_ub) {
			OLED_ShowString(80, 2, (uint8_t*)"L");
		}
		else { // 处于两阈值之间
			OLED_ShowString(80, 2, (uint8_t*)"Y"); // 表示正常
		}
		/*Temprature 操作结束*/
		
		/*Lux 操作*/
		if(E53_IA1_Data.Lux < lux_lb) { // 如果光照小于最低阈值 将补光灯开启 并将所有LED灯打开
			E53_IA1_Light_StatusSet(ON);
			all_led_on;
			OLED_ShowString(80, 4, (uint8_t*)"L");
		}
		else if(E53_IA1_Data.Lux > lux_ub) {
			E53_IA1_Light_StatusSet(OFF);
			all_led_off;
			OLED_ShowString(80, 4, (uint8_t*)"H");
		}
		else { // 处于两阈值之间
			E53_IA1_Light_StatusSet(OFF);
			all_led_off;
			OLED_ShowString(80, 4, (uint8_t*)"Y"); // 表示正常
		}
		/*Lux 操作结束*/
		
		if(E53_IA1_Data.Temperature > temp_ub
		|| E53_IA1_Data.Temperature < temp_lb
		|| E53_IA1_Data.Humidity > humd_ub
		|| E53_IA1_Data.Humidity < humd_lb) {
			BEEP_LONG_3();  //long beep for 3 times
			PWM_ON();
			BEEP_SHORT_3(); //short beep for 3 times
		} else {
			E53_IA1_Motor_StatusSet(OFF);
			BEEP(100); // long beep once
		}
		
		// suspend for a while
		delay_1ms(400);
	}
}
